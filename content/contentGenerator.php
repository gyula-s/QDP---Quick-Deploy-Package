...
function readArticles($folder, $articleFileName){
    ...
    if($folder == "404"){
        include_once(rootFolder.'/helpers/errorPages.php');
        displayError("404");
        return;
    } elseif ($folder == "401") {
        include_once(rootFolder.'/helpers/errorPages.php');
    ...
    if ($folder == "root" or $folder == "wwwroot" or $folder == ""){
	   $folder = siteLanguage.DS.$langSettings["homeName"]; //when the folder variable is root, wwwroot or empty assume the home folder
    }

    //create a variable with the exact location of the articles needed

    $location = rootFolder.DS."content".DS.siteLanguage.DS.$folder;
    ...
    if (file_exists($location.DS.$articleTitle)){
        ...
        if($article['title'] == "insertContactForm"){
            ...
            if($article['titleEnabled']){
                echo "\n\t\t<div class='title'><h1>";
                echo '<a href='.siteDomain.urlencode($folder);
                echo '/';
                echo urlencode(substr($articleTitle, 0, -5)).'>';
